...
class PeerClient:
    def __init__(self, ip, port, available_pieces):
        ...
        self.ip = ip
        self.port = port
        self.available_pieces = available_pieces  # {file_name: set(piece_ids)}
...
class TorrentDownloader:
    def __init__(self, peers, files):
        """
        Initialize with a list of PeerClient objects and a dictionary of files with their needed pieces.
        
        Parameters:
        - peers: List of PeerClient objects
    ...
    def download_file(self, file_name, pieces_needed):
        ...
        while pieces_needed:
            rarest_pieces = self.get_rarest_pieces(file_name, pieces_needed)
    ...
    def get_rarest_pieces(self, file_name, pieces_needed):
    ...
    def start_downloading(self):
        ...
        # Close each peer connection after downloads are complete
        for peer in self.peers:
...
if __name__ == "__main__":
    # Define which peers have which pieces
    peers = [
        PeerClient("127.0.0.1", 33357, {"file1": {1, 2, 3}, "file2": {1, 2}}),
        PeerClient("127.0.0.1", 33358, {"file1": {3, 4, 5}, "file2": {2, 3, 4, 5}}),
        PeerClient("127.0.0.1", 33359, {"file1": {2, 3, 5}, "file2": {1, 4, 5}}),
    ]
    ...
    downloader = TorrentDownloader(peers, files)
